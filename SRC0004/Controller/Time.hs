...
ammo = durDisplay} 
    ...
    where
      ...
      newPs               = 
        concatMap (explosion 1 ((abs $ fst $ head randomList) / 200.0 + 0.1) 10 0.5  . pos . eObj) (hitEs)++
        concatMap (explosion 1 ((abs $ fst $ head randomList) / 200.0 + 0.1) 3  0.25 . pos . eObj) ((snd.safeEs) es')
