...
draw horizontalResolution verticalResolution world@(World{..})
  ...
  where 
    ...
    --HuD
    ammo' = map (\p -> pColor p $ Translate (x p) (y p) $ rect 1 1) 
                ammo
              where
                x = fst . pos . pObj  
                y = snd . pos . pObj 
    ...
    -- Enemies
    es'      = map (\e -> white $ Translate (x e) (y e) $ Rotate (a e) $ ePath e) enemies
              where
                x = fst . pos . eObj
                y = snd . pos . eObj
                a = (\x -> fst x - snd x) . pos . eObj
                ePath e = Line $ map (mulSV 8) $ asteroidPaths !! mod (eType e) (length asteroidPaths)
    -- Particles
