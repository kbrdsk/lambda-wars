  ...
  do
    ...
    let ret = case state of
              0 -> case shootAction of
                    ...
                    _     -> case rotateAction of
                             NoRotation   -> world{particles = ps', velocity = (0, 1), backgrounds = bgs, bgParticles = pMap, upgrade = Item (Object (1000,0) zv) 10 1 False, angle = pi/2}
                             RotateLeft   -> world{menuState = max 0 (menuState - 1), particles = ps', velocity = (0, 1), backgrounds = bgs, bgParticles = pMap, upgrade = Item (Object (1000,0) zv) 10 1 False, angle = pi/2}
                             RotateRight  -> world{menuState = min 5 (menuState + 1), particles = ps', velocity = (0, 1), backgrounds = bgs, bgParticles = pMap, upgrade = Item (Object (1000,0) zv) 10 1 False, angle = pi/2}
...
ammo = durDisplay} 
    ...
    where
      ...
      newE          = [Enemy (Object (addV ran1 (rotateV (argV ran1) (400,0))) 
                        (0,0)) (eDifficulty+1) eDifficulty (floor $ (abs $ fst $ head randomList)/400 * 4) | length es' < 10]
      ...
      newPs               = 
        concatMap (explosion 1 ((abs $ fst $ head randomList) / 400.0 + 0.05) 10 0.5  . pos . eObj) (hitEs)++
        concatMap (explosion 1 ((abs $ fst $ head randomList) / 400.0 + 0.05) 3  0.25 . pos . eObj) ((snd.safeEs) es')
      deadPs              = explosion 0 ((abs $ fst $ head randomList) / 800.0) 20 0.5 (0,0)
      dyingP p            = p {pLifeTime = pLifeTime p - time}
      explosion y n s l v = [Particle (Object v ((snd x) * s * cos (fst x), (snd x) * s * sin (fst x))) (min 0.5 $ snd x) y |
                       x <- zip [0, n..10] (map (\x -> fst x / 400.0) randomList)]
                  
      -- trails
      trail   = Particle (Object (0,0) $ rotateV ((fst $ head $ randomList)/1200) $ mulSV (-1) vel') 1 3
