...
timeHandler w h time world@(World{..}) = 
  do
    ...
    where
      ...
      newPs               = 
        concatMap (explosion 1 8 10 0.5  . pos . eObj) (hitEs)++
        concatMap (explosion 1 6 3  0.25 . pos . eObj) ((snd.safeEs) es')
      deadPs              = explosion 0 16 20 0.5 (0,0)
      dyingP p            = p {pLifeTime = pLifeTime p - time}
      explosion y n s l v = [Particle (Object v (s * cos x, s * sin x)) l y |
                       x <- [0, (pi/n)..(2*pi)]]
