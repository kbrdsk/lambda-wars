...
draw horizontalResolution verticalResolution world@(World{..})
  = 
  -- Choose rendering function according to state
  case state of
    0 -> return render'
    1 -> return render
    2 -> return render''
  where 
    -- gameplay loop
    render   = pictures $ pulse++ammo'++bgps'++is'++es'++ps'++
                (score':mult:lives':bs' ++ ship)
    -- menu
    ...
    menuBg    = Color (blackP 0.5)  $ rect (sw * 4) (sh * 4)
    menuTitle = Translate (-sw/3 - sw/25)         (sh/1.5) 
                  $ scale 0.4 0.4   $ Color (crimsonP 1)  $ text menuT
    menuDiff  = Translate (-sw/3 + sw/4- sw/24)   (-verticalResolution/2 + 100) 
                  $ scale 0.15 0.15 $ Color white         $ text menuD
    menuPlay  = Translate (-sw/3 + sw/11- sw/20)  (-verticalResolution/2 + 50) 
                  $ scale 0.2 0.2   $ Color white         $ text menuP
    menuHS    = Translate (-sw/3 + sw/5- sw/24)   (sh/3) 
                  $ scale 0.2 0.2   $ Color white         $ text menuH
    ...
    -}
   -- gTime    = Translate 385 190 $ scale 0.1 0.1 $ white $ text $ show globalTime
    score'   = Translate (-385) 230 $ scale 0.1 0.1 
                (Color white $ text (show score))
    mult     = Translate (-385) 210 $ scale 0.1 0.1 
                (Color white $ text ("x"++(show $ floor (fromIntegral multiplier / 10))))
    ...
    --HuD
    ammo' = map (\p -> Color (iColor (Item (Object (0,0) (0,0)) 20 weapon True)) $ Translate (x p) (y p) $ circleSolid 4) 
                ammo
              where
                x = fst . pos . pObj  
                y = snd . pos . pObj 
    ...
    -- Items
    is'      = map (\i -> Color (iColor i) $ Translate (x i) (y i) $ thickCircle (radius i) 3) 
                (upgrade:bonus)
             where
              x = fst . pos . iObj
              y = snd . pos . iObj
    ...
    radius i = if pickedUp i then 10 + (0.25 - lt) * 100 else 10
              where
                lt = (min (iLifeTime i) 1.0)
    pulse    = map (\ugp -> Color (iColor ugp) $ Translate (fst (pos (iObj upgrade))) (snd (pos (iObj upgrade))) $ thickCircle (radius ugp - 15) 1) upgradePulse
