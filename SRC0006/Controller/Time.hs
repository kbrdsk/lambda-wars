...
timeHandler w h time world@(World{..}) = 
  do
    ...
    if newHigh /= highScore then
      (do handle <- openFile "high.score" ReadWriteMode
          mapM (hPrint handle) highScoreList'
          hClose handle
          return ())
      else return ()
    let ret = case state' of
              0 -> case shootAction of
                    Shoot -> world{state = 1, lives = 3} --
                    _     -> world{particles = ps', velocity = (0, 1),
                             backgrounds = bgs, bgParticles = pMap, 
                             upgrade = Item (Object (1000,0) zv) 10 1 False,
                             angle = pi/2, enemies = [], state = state',
                             score = newScore}
              1 -> world{enemies = es, bullets = bs, bonus = bonus', 
                    ...
                    ammo = durDisplay, globalTime = globalTime',
                    upgradePulse = pulse} 
              2 -> world{particles = ps', globalTime = globalTime',
                         state = state', enemies = es, bullets = []}
    return ret
    where
      -- constants
      state'       | gameOver && (state /= 2) = 0 
                   | globalTime == 0 = 1
                   | crash && state == 1 = 2
                   | otherwise = state
      theta        = pi/40
      ...
      -- variables
      ang   = if state == 2 then pi/2 else angle + case rotateAction of
              RotateLeft  -> theta
              RotateRight -> -theta
              NoRotation  -> 0
      acc   = case movementAction of 
              Thrust      -> (0.15, 0) 
              NoMovement  -> (0,0)
      globalTime' | crash && state == 1 = 99900
                  | otherwise           = mod (globalTime + 1) 100000 
      ...
      crash 
        | state == 1   = any (col player 25 . eObj) enemies && globalTime > 200
        | otherwise    = False
  
      -- player   
      vel'   | state == 2  = (0,0) 
             | otherwise   = mulSV decay $ addV velocity $ rotateV ang acc   
      ...
      getParticles m = 
        let (x,y)    = mapLoc m
            ...
            createPs  (px,py) = 
              map (translateSecP (px, py)) (localPMap (px, py) m)
      ...
      -- enemy updater
      es            =
        filter (\e -> magV (pos (eObj e)) < 550) $
                newE ++ map eGT ((fst.safeEs) es')
      newE          = 
        [Enemy (Object (addV ran1 (rotateV (argV ran1) (400,0))) 
        (0,0)) (eDifficulty+1) eDifficulty 
        (floor $ abs (fst $ head randomList)/400 * 4) 
        | length es' < 10]
      ...
      safeEs []     = ([], [])
      safeEs (x:xs) = 
        (x:fst splitEs, snd splitEs ++ fst eDivide)
         where eDivide = 
                partition ((\o -> col (eObj x) 13 o
                ||(state == 2 && globalTime < 99901
                 && globalTime > 0 && col player 300 o)).eObj) xs
      ...
      -- weapon upgrade
      gotUpgrade  = pickedUp upgrade
      upgrade'    = dyingI $ iPicked upG'{iObj = gMove (iObj upG')}
      upG'        = 
        if iLifeTime upgrade < 0 || gotUpgrade || state == 2
          || magV (pos (iObj upgrade)) > 1000 then 
          Item (Object (addV ran2 (rotateV (argV ran2) (200,0))) (0,0)) 20 
          (mod (iType upgrade) 3 + 1) False
        else upgrade
      pulse       = 
        [Item (iObj upgrade)
        (fromIntegral (200 - mod (globalTime+50*x) 150)/2000)
        (iType upgrade) True | x <- [1..3]]
      durDisplay  | null ammunition || state == 2 = []
                  | otherwise                = case (shootAction, cooldown) of
                                              (Shoot, 0) -> tail ammunition
                                              (_, _)     -> ammunition
      ammunition = 
        if gotUpgrade then  
        [Particle (Object (addV (50 * cos x, 50 * sin x) (400,-200)) 0)
                   1 (iType upgrade) | x <- [(2*pi/n), 4*pi/n..(2*pi)]] 
        else ammo
        where n = fromIntegral $ weapDur $ iType upgrade

      -- bullet updater
      bs            = if state==2 then [] else newB ++ bsMod bullets
      ...
      bullet        = case weapon of
                      0 -> [bCreator 15 angle 1]
                      1 -> [bCreator 15 angle 1]
                      2 -> [bCreator 15 (angle + x*theta) 1 | x <- [-1, 0, 1]]
                      3 -> [bCreator 15 x 1 | x <- [0, pi/4 .. 2*pi]]
      ...
      newPs               = 
        concatMap 
        (explosion 1 ((abs $ fst $ head randomList) / 400.0 + 0.05) 10 0.5  
          . pos . eObj) hitEs++
        concatMap
         (explosion 1 ((abs $ fst $ head randomList) / 400.0 + 0.05) 3  0.25
          . pos . eObj) ((snd.safeEs) es')
      deadPs              = 
        explosion 0 ((abs (fst $ head randomList)) / 800.0) 20 0.5 (0,0)
      dyingP p            = p {pLifeTime = pLifeTime p - time}
      explosion y n s l v = 
        [Particle 
        (Object v (snd x * s * cos (fst x), 
                  snd x * s * sin (fst x))) 
                  (min 0.5 $ snd x) y 
                  | x <- zip [0, n..10] (map (\x -> fst x / 400.0) randomList)]
                  
      -- trails
      trail   = Particle (Object (0,0) $
                rotateV (fst (head randomList)/1200) $
                mulSV (-1) vel') 1 3
      ps'     = case movementAction of
              Thrust      -> if state == 1 then trail:ps else ps
              NoMovement  -> ps
  
      -- item updater
      -- bonus multiplier
      bonus'   = if state == 2 then [] else newI ++ filter iCheck 
                  (map ((\i -> i{iObj = gMove (iObj i)}).dyingI.iPicked) bonus)
      dyingI i = i {iLifeTime = iLifeTime i - time}
      iPicked i = if col (iObj i) pickUpRadius player && not (pickedUp i) 
                  then i {pickedUp = True, iLifeTime = 0.25} else i
      iCheck i = iLifeTime i > 0  && 
                  magV (pos (iObj i)) < 1000
      newI     = 
        [Item 
        (Object (addV ran2 (rotateV (argV ran2) (400,0))) (0,0)) 10 0 False 
        | length bonus < 3]

      -- score updater
      newScore = if gameOver then 0 
                 else score + multiplier * sum (map eScore hitEs)
      newMult  
       | state == 2 = 10 -- col (Object (0,0) (0,0)) pickUpRadius (iObj x) && not (pickedUp x)
       | any (\x -> pickedUp (iPicked x) && iLifeTime (iPicked x) == 0.25) bonus
           = multiplier + 20 
       | otherwise = multiplier
      newHigh  = if gameOver && score > highScore then score else highScore
      highScoreList' = 
        if gameOver then take 10 $ sortBy (flip compare) (score:highScoreList)
        else highScoreList
